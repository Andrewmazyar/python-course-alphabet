...
def task_5_delete_the_last_customer(con) -> None:
    ...
    """
    with con.cursor() as cur:
        cur.execute('DELETE Customer WHERE CustomerID = MAX')
...
def task_6_list_all_supplier_countries(cur) -> list:
    ...
    """
    cur.execute("SELECT * FROM Customers WHERE Country = supplier; ")
    return cur.fetchall()
...
def task_7_list_supplier_countries_in_desc_order(cur) -> list:
    ...
    """
    cur.execute("SELECT country FROM supplier ORDER BY Country DESC;")
    return cur.fetchall()
...
def task_12_list_suppliers_from_specified_countries(cur):
    ...
    Returns: 8 records
    """
    cur.execute("SELECT FROM supplier WHERE Country = 'USA' AND Country = 'UK' AND Country = 'Japan';")
    return cur.fetchall()
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    Returns: 3 records
    """
    cur.execute('SELECT products FROM supplier WHERE country = Sweden;')
    return cur.fetchall()
...
def task_14_list_products_with_supplier_information(cur):
    ...
    Suppliers.City, Suppliers.SupplierName FROM Products INNER JOIN Suppliers 
    ON Products.SupplierId = Suppliers.SupplierId WHERE SupplierName IS NOT NULL''')
    return cur.fetchall()
...
def task_15_list_customers_with_any_order_or_not(cur):
    ...
    Returns: 213 records
    """
    cur.execute("SELECT Customers.CustomerName, Customers.ContactName, Customers.Country, Orders.OrderId FROM Customers"
...
def task_16_match_all_customers_and_suppliers_by_country(cur):
    ...
    Returns: 194 records
    """
    cur.execute('"SELECT * FROM Customers INNER JOIN Suppliers ON Customers.ContactName = Suppliers.ContactName;')
    return cur.fetchall()
